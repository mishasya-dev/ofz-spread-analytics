...
## v0.4.1 — Исправление выбора облигаций (13.03.2026)

### Проблема

При обновлении страницы выбор пары облигаций менялся:
1. Пользователь выбирает пару SU26238/SU26235
2. Обновляет страницу
3. Выбор меняется на SU26221/SU26225

### Причина

`selected_bond1` и `selected_bond2` хранились как **индексы** в списке облигаций.
Но список формировался из `dict.values()` с **непредсказуемым порядком**.

```python
# Проблемный код:
st.session_state.selected_bond1 = 0  # индекс
bonds = list(st.session_state.bonds.values())  # порядок случаен!
```

При каждом запуске порядок элементов мог меняться → индексы указывали на другие облигации.

### Решение

1. **Хранить выбранные облигации по ISIN:**
   ```python
   st.session_state.selected_bond1_isin = "SU26238RMFS4"
   st.session_state.selected_bond2_isin = "SU26235RMFS0"
   ```

2. **Гарантировать стабильный порядок списка:**
   ```python
   # Сортировка по duration_years
   sorted_bonds = sorted(bonds_dict.values(), key=get_duration)
   ```

3. **Находить индекс по ISIN при рендеринге:**
   ```python
   bond1_idx = bond_isins.index(selected_isin1)
   ```
...
| Файл | Изменение |
|------|-----------|
| `app.py` | `selected_bond1_isin` вместо `selected_bond1` |
| `app.py` | `get_bonds_list()` с сортировкой по duration |
| `app.py` | Callback-функции для синхронизации ISIN |

### Контекстные менеджеры (дополнительно)

Также в этой сессии были переписаны все файлы на использование контекстных менеджеров:

| Файл | Исправлений |
|------|-------------|
| `core/db/facade.py` | 15 использований → `get_db_connection()`/`get_db_cursor()` |
| `core/ofz_cache.py` | 4 использования |
| `tests/test_bonds.py` | 4 использования |
| `tests/test_sidebar.py` | 1 использование |

### Git

- Ветка: `feature/g-spread-yearyields-method`
- Коммиты:
  - `5dd9421` — refactor: Переход на контекстные менеджеры
  - `772f960` — fix: Сохранение выбора облигаций по ISIN

---

## v0.4.2 — Сохранение настроек в URL (13.03.2026)

### Проблема

При F5 (обновлении страницы) сбрасывались:
- Выбранная пара облигаций
- Значения слайдеров (период, окно, z-threshold)

### Решение

Настройки сохраняются в **URL-параметры**:
```
app/?b1=SU26238RMFS4&b2=SU26235RMFS0&p=365&sw=30&zt=2.0&...
```

**Плюсы:**
- Каждое окно браузера = независимые настройки
- F5 сохраняет настройки (они в URL)
- Можно открыть несколько окон с разными парами
- Можно скопировать ссылку и поделиться
...
| `b1` | ISIN | Облигация 1 |
| `b2` | ISIN | Облигация 2 |
| `p` | 30-730 | Период анализа |
| `ci` | 1/10/60 | Интервал свечей |
| `cd` | 1-365 | Период свечей |
| `sw` | 5-90 | Spread window |
| `zt` | 1.0-3.0 | Z-threshold |
| `gp` | 30-730 | G-spread период |
| `gw` | 5-90 | G-spread window |
| `gzt` | 1.0-3.0 | G-spread z-threshold |

### Реализация

```python
# Чтение из URL при старте
...
def save_settings_to_url():
    st.query_params.from_dict({
        "b1": st.session_state.selected_bond1_isin,
        "b2": st.session_state.selected_bond2_isin,
        "p": str(st.session_state.period),
        # ...
    })
```
