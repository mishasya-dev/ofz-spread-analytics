...
# Российские праздники 2024-2025
HOLIDAYS_2024 = [
...
def check_by_api(board: str = "TQOB", client: MOEXClient = None) -> Dict[str, Any]:
    """
...
def check_by_schedule(check_time: Optional[datetime] = None) -> ExchangeInfo:
    """
    ...
    if current_time < PREMARKET_START:
        return ExchangeInfo(
            ...
            current_time=check_time,
            session_start=datetime.combine(current_date, MAIN_SESSION_START)
        )

    if PREMARKET_START <= current_time < MAIN_SESSION_START:
        return ExchangeInfo(
    ...
    if MAIN_SESSION_START <= current_time < MAIN_SESSION_END:
        return ExchangeInfo(
            ...
            current_time=check_time,
            session_start=datetime.combine(current_date, MAIN_SESSION_START),
            session_end=datetime.combine(current_date, MAIN_SESSION_END)
        )

    if POSTMARKET_START <= current_time < POSTMARKET_END:
        return ExchangeInfo(
    ...
    return ExchangeInfo(
        ...
        current_time=check_time,
        next_session=_get_next_session_date(check_time)
    )


def check_comprehensive(board: str = "TQOB", client: MOEXClient = None) -> ExchangeInfo:
    """
...
def is_trading_now(board: str = "TQOB", client: MOEXClient = None) -> bool:
    """
    ...
    """
    info = check_comprehensive(board, client)
    return info.is_trading


def get_trading_schedule(check_date: Optional[date] = None) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "main_session_start": MAIN_SESSION_START.isoformat(),
        "main_session_end": MAIN_SESSION_END.isoformat(),
        "postmarket_start": POSTMARKET_START.isoformat(),
        "postmarket_end": POSTMARKET_END.isoformat(),
        "is_weekend": check_date.weekday() >= 5,
        "is_holiday": check_date.isoformat() in ALL_HOLIDAYS
    }


def _get_next_session_date(current: datetime) -> datetime:
    """
    ...
    for _ in range(7):  # Максимум 7 дней
        ...
        if next_date.isoformat() in ALL_HOLIDAYS:
            continue

        break

    return datetime.combine(next_date, MAIN_SESSION_START)
