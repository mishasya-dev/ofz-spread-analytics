...
@pytest.fixture
def service(mock_client, mock_ytm_repo):
    """CandleService с моками"""
    return CandleService(client=mock_client, ytm_repo=mock_ytm_repo)
...
class TestCandleServiceInit:
    ...
    def test_init_with_client_and_repo(self, mock_client, mock_ytm_repo):
        """Инициализация с client и repo"""
        service = CandleService(client=mock_client, ytm_repo=mock_ytm_repo)
        
        assert service._client is mock_client
        assert service._ytm_repo is mock_ytm_repo
    
    def test_init_without_client(self, mock_ytm_repo):
        """Инициализация без client (ленивая загрузка)"""
        service = CandleService(ytm_repo=mock_ytm_repo)
        
        assert service._client is None
        # client создаётся при обращении
        assert service.client is not None
    
    def test_init_without_repo(self, mock_client):
        ...
        with patch('services.candle_service.YTMRepository') as MockRepo:
            ...
            service = CandleService(client=mock_client)
...
class TestFetchTodayCandles:
    ...
    def test_fetch_today_success(self, service, mock_client, sample_bond, sample_candles_df):
        """Успешная загрузка сегодняшних свечей"""
        with patch('services.candle_service.fetch_candles', return_value=sample_candles_df):
            with patch.object(service, 'ytm_processor') as mock_processor:
                mock_processor.add_ytm_to_candles.return_value = sample_candles_df
                
                result = service._fetch_today_candles(sample_bond, "60")
                
                assert len(result) == 10
    
    def test_fetch_today_exception(self, service, sample_bond):
        """Обработка исключения при загрузке"""
        with patch('services.candle_service.fetch_candles', side_effect=Exception("API Error")):
            result = service._fetch_today_candles(sample_bond, "60")
...
class TestFetchHistoryCandles:
    ...
    def test_fetch_history_success(self, service, sample_bond, sample_candles_df):
    ...
    def test_fetch_history_exception(self, service, sample_bond):
...
class TestClose:
    ...
    def test_close_with_client(self, service, mock_client):
        """Закрытие с client"""
        service.close()
        mock_client.__exit__.assert_called_once()
    
    def test_close_without_client(self, mock_ytm_repo):
        """Закрытие без client не вызывает ошибку"""
        service = CandleService(client=None, ytm_repo=mock_ytm_repo)
